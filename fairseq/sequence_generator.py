...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        bert_encoder_padding_mask = bertinput.eq(model.models[0].berttokenizer.pad_token_id)
        # _, _, bert_outs = model.models[0].bert_encoder(bertinput, attention_mask=~bert_encoder_padding_mask)
        # bert_outs = bert_outs[1:]
        # bert_outs = bert_outs[self.bert_output_layer]
        # bert_outs = bert_outs.permute(1, 0, 2).contiguous()
        bert_outs = model.models[0].forward_bert(bertinput, bert_encoder_padding_mask)
        if model.models[0].mask_cls_sep:
            bert_encoder_padding_mask += bertinput.eq(model.models[0].berttokenizer.cls_token_id)
            bert_encoder_padding_mask += bertinput.eq(model.models[0].berttokenizer.sep_token_id)
        ...
        if model.models[0].__class__.__name__ == 'BertTransformerModel':
