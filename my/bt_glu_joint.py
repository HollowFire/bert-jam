...
from .joint_attention import JointMultiheadAttention

@register_model('bt_glu_joint')
class BertTransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward_bert(self, bert_input, bert_encoder_padding_mask):
        ...
        bert_encoder_out = list(bert_encoder_out[1:])
        for i in range(len(bert_encoder_out)):
            bert_encoder_out[i] = bert_encoder_out[i].permute(1,0,2).contiguous().unsqueeze(-1)
        bert_encoder_out = torch.cat(tuple(bert_encoder_out), dim=-1) # T, B, C, L
...
class BertTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(self, prev_output_tokens, encoder_out=None, bert_encoder_out=None, incremental_state=None, **unused):
        ...
        tgt_padding_mask = prev_output_tokens.eq(self.padding_idx) # (B, T)
...
class BertTransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, bert_gate=True):
        ...
        self.bert_enc_attn = JointMultiheadAttention(
            embed_dim=self.embed_dim,
            key_embed_dim=args.bert_out_dim,
        ...
        self.glu_linear = nn.Linear(args.bert_num_layers, 2, bias=False)
        self.glu = nn.GLU()
        self.reset_parameters(args.bert_num_layers-1)
        self.recover_bert = 1. if args.adjust_layer_weights else 2.

    def reset_parameters(self, layer_index):
        nn.init.zeros_(self.glu_linear.weight)
        with torch.no_grad():
            self.glu_linear.weight[0, layer_index] = 1.
    ...
    def forward(self, x, encoder_padding_mask, bert_encoder_out, bert_encoder_padding_mask):
        ...
        """
        bert_source = bert_encoder_out
        bert_source = self.glu_linear(bert_source)
        bert_source = self.glu(bert_source)
        bert_source = bert_source.squeeze(-1) * self.recover_bert

        residual = x
        x, _ = self.bert_enc_attn(
            query=x,
            key=bert_source,
...
class BertTransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, bert_gate=True):
        ...
        self.enc_dec_attn = JointMultiheadAttention(
            embed_dim=self.embed_dim,
            key_embed_dim=args.encoder_embed_dim,
            num_heads=args.decoder_attention_heads,
            dropout=args.attention_dropout
        )

        self.bert_dec_attn = JointMultiheadAttention(
            embed_dim=self.embed_dim,
            key_embed_dim=args.bert_out_dim,
            num_heads=args.decoder_attention_heads,
            dropout=args.attention_dropout
    ...
    def forward(
        ...
        if incremental_state is None:
            ...
            bert_source = self._get_cached_bert(incremental_state)
            if bert_source is None:
                bert_source = bert_encoder_out
                bert_source = self.glu_linear(bert_source)
                bert_source = self.glu(bert_source)
                bert_source = bert_source.squeeze(-1) * self.recover_bert
                self._set_cached_bert(incremental_state, bert_source)

        residual = x

        x1, attn = self.enc_dec_attn(
            query=x,
            key=encoder_out,
    ...
    def _set_cached_bert(self, incremental_state, buffer):
...
@register_model_architecture('bt_glu_joint', 'bt_glu_joint')
...
@register_model_architecture('bt_glu_joint', 'bt_glu_joint_iwslt_de_en')
def bt_glu_joint_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('bt_glu_joint', 'bt_glu_joint_wmt_en_de_big')
def bt_glu_joint_wmt_en_de_big(args):
